...
{
    cout << "usage : inputdir framenameref framename extension loop_start loop_end loop_half_window" << endl;
    cout << "> ./looping ../images lionwalk lionwalkprocessed png 94 120 4" << endl;
    cout << endl;
    char *inputdir=             argv[1];
    char *framenameref=         argv[2];
    char *framename=            argv[3];
    char *extension=            argv[4];
    int loop_start =            atoi(argv[5]);
    int loop_end =              atoi(argv[6]);
    int loop_half_window =      atoi(argv[7]);
    string input;
    int flowmethod=1;
    ...
    char linuxcmd[1024];
    Mat imageA,imageB,imageAref,imageBref,simageA,simageB,bwA,bwB,mapx,mapy,warpA,warpB,showflowbg,finalframe;
    ...
    for (int i = transition_start + 1; i < transition_end; i++) {
        ...
        sprintf(Aimageref,"%s/%s.%04d.%s",inputdir,framenameref,Aframe,extension);
        sprintf(Bimageref,"%s/%s.%04d.%s",inputdir,framenameref,Bframe,extension);
        cout << "processing loop frame : " << i-loop_cut+1 << endl;
        //read A/B image
        ...
        cout <<"readingA ref : " << Aimageref << endl;
        imageAref= imread(Aimageref,IMREAD_COLOR);
        cout <<"readingB ref : " << Bimageref << endl;
        imageBref= imread(Bimageref,IMREAD_COLOR);
        ...
        if (flowmethod != 0) {
            ...
            if (flowmethod == 1) { //use opencv deep optical flow
                ...
                cvtColor(imageAref, bwA, COLOR_BGR2GRAY);
                cvtColor(imageBref, bwB, COLOR_BGR2GRAY);
            ...
            if (flowmethod == 4) { //use simpleflow optical flow
                ...
                //resize for optical flow computation
                if (flowscale != 1) {
                    cv::resize(imageAref,simageA,cv::Size(),flowscale,flowscale,INTER_AREA);
                    cv::resize(imageBref,simageB,cv::Size(),flowscale,flowscale,INTER_AREA);
                    ...
                    BAflow = Mat(imageAref.size[0], imageAref.size[1], CV_32FC2);
                    ABflow = Mat(imageBref.size[0], imageBref.size[1], CV_32FC2);
                    //process optical flow
                    cout << "processing B->A flow  : " << Bframe << "->" << Aframe << " [useGpu : " << int(useGpu && cv::ocl::haveOpenCL()) << "]" << endl;
                    if (useGpu) algorithm->calc(imageBref, imageAref, BAflow.getUMat(ACCESS_RW));
                    else algorithm->calc(imageBref, imageAref, BAflow);
                    cout << "processing A->B flow  : " << Aframe << "->" << Bframe << " [useGpu : " << int(useGpu && cv::ocl::haveOpenCL()) << "]" << endl;
                    if (useGpu) algorithm->calc(imageAref, imageBref, ABflow.getUMat(ACCESS_RW));
                    else algorithm->calc(imageAref, imageBref, ABflow);
                }
                //resize flow for warping
                if (flowscale != 1) {
                    cv::resize(BAflow,BAflow,cv::Size(),1/flowscale,1/flowscale,INTER_CUBIC);
            ...
            if (flowmethod == 10) {
                ...
                if (flowscale != 1) {
                    ...
                    cv::imwrite(ASimage,imageAref);
                    cv::imwrite(BSimage,imageBref);
                }
